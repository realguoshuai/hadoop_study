...
public class TimeHelper {
    ...
    public static String ChangeRecordTime2DocTime(String recordTime) {
        ...
        if (null != recordTime && recordTime.length() > 0) {
    ...
    /**
     * Description  将传入的String 类型的时间参数固定减去8小时
     * Param [dateStart, dateEnd]
     * return void
     **/
    public static String SubTime8Hour(String dateStart) {
        java.util.Date start = getDateTime(dateStart);
        /** GC日志特殊处理，将传入的时间参数固定减去8小时 */
        Calendar cal = Calendar.getInstance();
        cal.setTime(start);
        cal.add(Calendar.HOUR_OF_DAY, -8);
        start = cal.getTime();
       // System.out.println(getDateTimeString(start));
        return getDateTimeString(start);
    }

    public static void main(String[] args) throws ParseException {

        SubTime8Hour("2019-08-20 00:00:00");
